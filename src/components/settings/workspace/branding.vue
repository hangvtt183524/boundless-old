  ...
  .branding
    .branding__logo-section
      .branding__logo-section__logo-container(
        :class="{'branding__logo-section__logo-container--no-image': !logoData.url, 'branding__logo-uploading': isUploadingLogo}"
        ref="dropzoneElement"
        id="dropzone"
      )
    ...
    .branding__content
      .branding__content__upload-section
        ...
        .branding__content__upload-section__description {{ $t('v2.workspaces.settings.uploadLogoDescription') }}
        .branding__content__upload-section__actions
          ...
          input(
            type="file"
            ref="logoFileInput"
            @input="onLogoElementInput"
            hidden
          )
          base-button.base-button--blue--negative(
            v-if="selectedWorkspace.logo !== null"
...
export default {
  ...
  mounted () {
    if (this.selectedWorkspace.logo) {
      this.logoData = JSON.parse(JSON.stringify(this.selectedWorkspace.logo))
    }

    // Initialize dropzone element
    const vm = this
    let options = {
      url: `${API_URL}/workspace/${this.workspaceId}/logo/`,
      method: 'patch',
      paramName: 'logo_file',
  ...
  computed: {
    isChanged () {
      const originalData = this.selectedWorkspace.logo ? this.selectedWorkspace.logo : this.getEmptyLogoObject()
      return originalData.logo !== this.logoData.logo
    }
  },
  watch: {
    selectedWorkspace: {
  ...
  methods: {
    ...mapActions('workspace', ['updateWorkspaceLogo', 'deleteWorkspaceLogo']),
    getEmptyLogoObject () {
      return {
        logo_file: null
      }
    ...
    onRemoveLogoClick () {
      ...
      this.loading = true
      this.deleteWorkspaceLogo({
        id: this.selectedWorkspace.id
      }).then(res => {
        global.toastr['success'](this.$t('v2.workspaces.settings.logoRemoved'), this.$t('general.success'))
        this.logoData = this.getEmptyLogoObject()
      }).finally(() => {
        this.loading = false
      })
    },
    onLogoElementInput (event) {
      this.onLogoSelect(event.target.files[0])
    },
    onLogoSelect (file) {
      ...
      this.loading = true
      this.updateWorkspaceLogo({
        id: this.selectedWorkspace.id,
        formData: formData
      }).then(response => {
        this.onLogoUpdate(file)
      }).finally(() => {
        this.loading = false
      })
    },
    onLogoUpdate (dropzoneFile) {
      global.toastr['success'](this.$t('v2.workspaces.settings.logoSettingsUpdated'), this.$t('general.success'))

      // Reset drop zone element view state
