...
export default {
  ...
  mounted () {
    this.fetchSecurityComplianceChecks()
  },
  computed: {
    tableHeaders () {
      const tableHeaders = [
  ...
  methods: {
    ...mapActions('workspace/network', ['fetchMerakiSecurityComplianceChecks', 'updateMerakiSecurityComplianceCheck']),
    fetchSecurityComplianceChecks () {
      let filters = {
      ...
      this.loading = true
      this.fetchMerakiSecurityComplianceChecks(filters).then(response => {
    ...
    onActionClick (actionType, item) {
      if (actionType === 'details') {
        this.$router.push({ name: 'SecurityComplianceCheckDetail', params: { complianceCheckId: item.id } })
